...
class SungJuk2:
    def __init__(self, name, kor, eng, mat, tot=0, avg=0, mean=0, grd='가'):
        # 멤버에 접근제한 기능 부여
        # 멤버명 앞에 __를 추가하면 private 멤버로 선언됨
        self.__name = name
        self.__kor = kor
        self.__eng = eng
        self.__mat = mat
    ...
    def __str__(self): # 멤버출력함수 - 매직함수
        result = f'{self.__name} {self.__kor} {self.__eng} {self.__mat} {self.__tot} {self.__avg} {self.__mean} {self.__grd}'
        return result

    # setter getter 메서드 생성 I: Java 스타일
    ...
    # 멤버명 앞에 @를 붙여서 함수의 의미를 보조적으로 설명하는데 사용
    # decorator
    @property   # getter
    def name(self):
        return self.__name
...
class SungJukService:
    ...
    def readSungJuk(self):
        ...
        eng = int(input('영어는? '))
        mat = int(input('수학은? '))
        return SungJuk2(kor, eng, mat, name)
